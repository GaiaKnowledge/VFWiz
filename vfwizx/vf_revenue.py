...
def market(System=None, Crop=none):  # REQUIRES CHANGING FOR SCALABILITY
...
def yield(System=none, Crop=none):
    Ya = Ys * Ap * PARf * CO2f * (1-Fr) * Tf # Adjusted Plant Yield = Standard Yield x Plant Area x PAR factor
...
def income
    PI = Pp * Pi * Ya * Psr # Plant income = Plant price x Plant index x Adjusted plant yield x plant share rate
